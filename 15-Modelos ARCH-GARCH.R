...
Bitcoints <-  ts(Bitcoin$`Close/Dolar`, start = 2018, frequency = 365)

plot(Bitcoints)

#Verificar se a Série é Estacionaria

#Criar FAC  e FACP

acf(Bitcoin$`Close/Dolar`,lend=2, lwd=5,col="darkblue",main= "Função de Autocorrelação - FAC")              #Melhorando aspecto da FAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

pacf(Bitcoin$`Close/Dolar`,lend=60, lwd=5,col="darkblue",main= "Função de Autocorrelação Parcial - FACP")   #Melhorando aspecto da PAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

#Teste ADF
ur.df(Bitcoints, "none", lags = 1)
...
IntOrdem1 <- diff(Bitcoin$`Close/Dolar`)
